...
class ModelParallelCNN(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Conv2d(1, 16, kernel_size=5, stride=1, padding=2).to('cuda:1')
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.layer2 = nn.Conv2d(16, 32, kernel_size=5, stride=1, padding=2).to('cuda:0')
    ...
    def forward(self, x):
        x = self.layer1(x.to('cuda:1'))
        x = self.pool(F.relu(x))
        x = self.layer2(x.to('cuda:0'))
        x = self.pool(F.relu(x))
