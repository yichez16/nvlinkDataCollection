...
namespace detail {
  ...
  public:
    ...
    profiler(const strvec_t& events,
      ...
      CUPTI_CALL(cuptiActivityEnable(CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL)); // CUPTI_ACTIVITY_KIND_KERNEL or CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL
...
//      cuCtxGetCurrent(&m_context);
    ...
    //     template<typename stream>
    // void print_start_duration(stream& s,
    //                         timeval ts,
    //                         timeval te) {
    //   using ull_t = unsigned long long;

    //         s << ts.tv_sec*1000000 + ts.tv_usec
    //           << "," << (te.tv_sec - ts.tv_sec)*1000000 + te.tv_usec - ts.tv_usec
    //           << ",";
    //   printf("\n");
    // }
    ...
    template<typename stream>
    void print_metric_values_elapsed(stream& s,
                            timeval ts,
                            timeval te,
                            timeval t0,
                             bool print_names=false
